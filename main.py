...
parser.add_argument('-cam', '--virtual-camera', action='store_true', default=False,
                    help="Stream to a virtual camera device.")
parser.add_argument('-ext', '--picture-extension', default="jpg", type=str,
                    help="Image type to be saved.")
...
if args.virtual_camera:
    cam = pyvirtualcam.Camera(width, height, args.fps, fmt=PixelFormat.BGR, print_fps=True)
...
while True:
    for q in q_list:
        ...
        if capture_flag:  # Save to disk if `c` was pressed
            filename = capture_file_info_str + "." + args.picture_extension.lstrip(".")
